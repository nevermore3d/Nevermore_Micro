...
Since the fan (probably) cant be reused for other projects and will function in a high temp environment affecting its lifetime, go for budget. The Sunon Maglev MF5015VX (high speed version, 6000 rpm) is good but unfortunately almost impossible to find as most are fakes (there is no 24V version for starters). The GDStime 6000rpm Dual Ball bearing is another good option, but quality may vary. The Delta BFB0524HH is a gucci option, as is the special micro versions for the papst rlf-35 which is both awesome and very expensive. Any fan that works well for stealthburner should be a good option for nevermore micro as well.
